...
export default function Layout({
  ...
  const navigation: Navigation[] = [...categoryNavItems, {
    ...
    items: collectionsData.length > 0 ? collectionsData.map(col => ({
      name: col.title,
      // Collections use 'title' property, not 'name'
      href: `/collections/${col.handle}`
    })) : [{
      name: 'Summer Shades',
      href: '/collections/summer-shades'
    }, {
      name: 'Luxury Line',
          ...
          <div className="flex h-full items-center justify-between">
            ...
            <div className="flex items-center">
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild className="lg:hidden mr-4">
                  <Button variant="ghost" size="icon" className={`transition-colors duration-300 ${hasScrolled ? 'text-foreground hover:bg-muted' : 'text-primary hover:bg-white/10'}`}>
                    <Menu className="h-5 w-5" />
                  </Button>
                </SheetTrigger>
                <SheetContent side="left" className="w-80 bg-background/95 backdrop-blur-md border-white/20">
                  <div className="flex flex-col gap-6 mt-8">
                    {navigation.map(item => <div key={item.name} className="space-y-3">
                        <Link to={item.href} className="text-lg font-semibold text-foreground tracking-wide transition-colors hover:text-primary" onClick={() => setMobileMenuOpen(false)}>
                          {item.name}
                        </Link>
                        {item.hasDropdown && item.items && <div className="ml-4 space-y-2">
                            {item.items.map(subItem => <div key={subItem.name} className="space-y-1">
                                <Link to={subItem.href} className="block text-sm font-medium text-foreground/80 hover:text-primary transition-colors" onClick={() => setMobileMenuOpen(false)}>
                                  {subItem.name}
                                </Link>
                                {subItem.children && subItem.children.length > 0 && <div className="ml-3 space-y-1">
                                    {subItem.children.map(child => <Link key={child.name} to={child.href} className="block text-xs text-foreground/60 hover:text-foreground transition-colors" onClick={() => setMobileMenuOpen(false)}>
                                        {child.name}
                                      </Link>)}
                                  </div>}
                              </div>)}
                ...
                animation: hasScrolled ? 'fade-in 0.6s ease-out forwards' : 'none'
                    ...
                    {item.hasDropdown ? <div className="relative">
                        ...
                        <div className="absolute top-full left-1/2 transform -translate-x-1/2 pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 ease-out translate-y-[10px] group-hover:translate-y-0 z-50">
                          <div className="bg-background/95 backdrop-blur-[12px] rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.4)] border border-white/10 w-[500px] py-4" style={{
                      backdropFilter: 'blur(12px)'
                    }}>
                            <div className="grid grid-cols-2 gap-4 px-4">
                              {item.items?.map(subItem => <div key={subItem.name} className="space-y-2">
                                  <Link to={subItem.href} className="block px-3 py-2 text-sm font-semibold text-foreground hover:text-primary hover:bg-white/10 rounded-lg transition-all duration-200" style={{
                            letterSpacing: '0.5px'
                          }}>
                                    {subItem.name}
                                  </Link>
                                  {subItem.children && subItem.children.length > 0 && <div className="space-y-1 ml-2">
                                      {subItem.children.map(child => <Link key={child.name} to={child.href} className="block px-3 py-1.5 text-xs text-foreground/70 hover:text-foreground hover:bg-white/5 rounded transition-all duration-200">
                                          {child.name}
                                        </Link>)}
                                    </div>}
                                </div>)}
                      ...
                      </div> : <Link to={item.href} className={`font-medium text-[10px] tracking-[0.15em] uppercase...`}>
                        {item.name}
                      </Link>}
                  </div>)}
              </div>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="flex items-center gap-3">
                ...
                {!searchExpanded ? <div className="hidden md:block overflow-hidden h-6 relative">
                    <div className={`transition-transform duration-500 ease-out ${hasScrolled ? 'text-foreground/60' : 'text-primary/60'}`} style={{
                  transform: `translateY(-${currentSearchPhrase * 24}px)`
                }}>
                      {popularSearchPhrases.map((phrase, index) => <div key={phrase} className="h-6 flex items-center text-xs font-light tracking-wide">
                          {phrase}
                        </div>)}
                    </div>
                  </div> : <div className="flex items-center transition-all duration-300 ease-out">
                    <Input type="search" placeholder="Search for sunglasses…" className={`w-64 h-8 text-sm transition-all duration-300 ease-out border-0 focus:ring-0 focus:border-0 focus:outline-none ${hasScrolled ? 'bg-background/50 text-foreground placeholder:text-foreground/60' : 'bg-white/10 text-primary placeholder:text-primary/60'}`} autoFocus />
                  </div>}
              ...
              }} className={`transition-all duration-500 ${hasScrolled ? 'text-foreground hover:bg-muted' : 'text-primary/80 hover:bg-white/10 hover:scale-110'}`}>
                  <Search className="h-4 w-4" />
                </Button>
              </div>

              {/* Full Search Overlay */}
              {fullSearchOpen && <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm animate-fade-in">
                  <div className="flex items-center justify-center min-h-screen p-4">
                    <div className="w-full max-w-2xl bg-background/90 backdrop-blur-[20px] border border-white/20 rounded-sm shadow-xl p-8 animate-scale-in">
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => setFullSearchOpen(false)} className="text-foreground/60 hover:text-foreground hover:bg-white/10">
                          <X className="h-5 w-5" />
                        </Button>
                      </div>
                      <Input type="search" placeholder="Search for sunglasses…" className="text-lg h-12 bg-white/5 border-white/20 text-foreground placeholder:text-foreground/60 focus:border-primary" autoFocus />
                    </div>
                  </div>
                </div>}
              ...
              {customer ? <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className={`transition-all duration-500 ${hasScrolled ? 'text-foreground hover:bg-muted' : 'text-primary/80 hover:bg-white/10 hover:scale-110'}`}>
              ...
              {/* Cart */}
              <CartDrawer>
                <Button variant="ghost" size="icon" className={`relative transition-all duration-500 ${hasScrolled ? 'text-foreground hover:bg-muted' : 'text-primary/80 hover:bg-white/10 hover:scale-110'}`}>
                  <ShoppingBag className="h-4 w-4" />
                  {itemCount > 0 && <Badge className={`absolute -top-1 -right-1 h-4 w-4 p-0 flex items-center justify-center transition-colors duration-500 ${hasScrolled ? 'bg-foreground text-background' : 'bg-primary text-background'}`}>
                      {itemCount}
                    </Badge>}
                </Button>
              </CartDrawer>
