...
export default function ProductDetail() {
  ...
  const {
    ...
    error: productError
  } = useProductDetail(handle || "", region?.id);
  ...
  const error = productError ? t('productDetail.notFoundOrFailed', {
  ...
  } = useRelatedProducts(product?.collection?.id || "", region?.id, 6);
    ...
    return <Layout>
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-foreground mb-4">{t('productDetail.productNotFound', {
              defaultValue: 'Product Not Found'
            })}</h1>
            <p className="text-muted-foreground mb-6">{error || t('productDetail.doesntExist', {
              defaultValue: "The product you're looking for doesn't exist."
            })}</p>
  ...
  return <>
    <Layout>
        ...
        <div className="w-full lg:w-[60%]">
          ...
          <div className="group relative w-full h-[360px] sm:h-[480px] md:h-[560px] lg:h-[720px] bg-muted/20 rounded-xl mb-4 flex items-center justify-center overflow-hidden">
            {product.images && product.images.length > 0 ? <>
                ...
                {/* Fullscreen button */}
                <button aria-label="Zoom image" onClick={() => setIsZoomOpen(true)} className="absolute top-3 right-3 z-10 inline-flex h-9 w-9 items-center justify-center rounded-full bg-background/70 text-foreground shadow-sm backdrop-blur hover:bg-background">
        ...
        <div className="w-full lg:w-[40%] space-y-4 sm:space-y-6 lg:space-y-8">
          ...
          <div className="mb-3 sm:mb-4 lg:mb-6">
            {price?.calculated_amount_with_tax ? <div className="flex items-baseline gap-3 sm:gap-4">
              ...
              </div> : <span className="text-lg text-muted-foreground">
                {t('productDetail.priceInCart', {
                  defaultValue: 'Price available in cart'
                })}
              </span>}
          </div>

          {/* Description */}
          {product.description && <div className="mb-4 md:mb-6">
          ...
          {/* Color Variants */}
          {product.variants && product.variants.length > 1 && <div className="mb-4 md:mb-6">
              <h4 className="text-lg font-medium text-foreground mb-3">{t('productDetail.availableOptions', {
                  defaultValue: 'Available Options'
                })}</h4>
              <div className="flex gap-3">
                {product.variants.map((variant, index) => <button key={variant.id} onClick={() => handleVariantSelect(variant)} className={`w-10 h-10 rounded-full border-2 transition-all ${selectedVariant?.id === variant.id ? 'border-foreground shadow-lg' : 'border-border hover:border-muted-foreground'}`} style={{
                  backgroundColor: variant.options?.find(opt => opt.option?.title?.toLowerCase() === 'color')?.value || `hsl(${index * 60}, 50%, 60%)`
                }} />)}
              </div>
            </div>}
          ...
          <div className="mb-4 md:mb-6">
            <h4 className="text-lg font-medium text-foreground mb-3">{t('productDetail.quantity', {
                  defaultValue: 'Quantity'
                })}</h4>
            <div className="flex items-center gap-3">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-muted transition-colors">
                <Minus className="w-4 h-4" />
              </button>
              <span className="text-lg font-medium w-8 text-center">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-muted transition-colors">
          ...
          {/* Add to Bag Button */}
          <Button onClick={handleAddToCart} disabled={isAddingToCart || !selectedVariant} className="w-full py-3 mb-4 md:mb-6 bg-foreground text-background hover:bg-foreground/90 font-semibold rounded-xl transition-all hover:shadow-xl">
            {isAddingToCart ? t('productDetail.adding', {
                defaultValue: 'Adding...'
          ...
          <Accordion type="single" collapsible className="mb-4 md:mb-6">
            <AccordionItem value="details">
              <AccordionTrigger className="text-lg font-medium">{t('productDetail.details', {
                    defaultValue: 'Product Details'
                  })}</AccordionTrigger>
              <AccordionContent>
                <div className="space-y-3 text-sm">
                  {product.weight && <div className="flex justify-between py-2 border-b border-border">
                      <span className="text-muted-foreground">{t('productDetail.weight', {
                          defaultValue: 'Weight'
                        })}:</span>
                      <span className="text-foreground font-medium">{product.weight}g</span>
                    </div>}
                  {product.length && <div className="flex justify-between py-2 border-b border-border">
                      <span className="text-muted-foreground">{t('productDetail.length', {
                          defaultValue: 'Length'
                        })}:</span>
                      <span className="text-foreground font-medium">{product.length}cm</span>
                    </div>}
                  {product.width && <div className="flex justify-between py-2 border-b border-border">
                      <span className="text-muted-foreground">{t('productDetail.width', {
                          defaultValue: 'Width'
                        })}:</span>
                      <span className="text-foreground font-medium">{product.width}cm</span>
                    </div>}
                  {product.height && <div className="flex justify-between py-2 border-b border-border">
                      <span className="text-muted-foreground">{t('productDetail.height', {
                          defaultValue: 'Height'
                        })}:</span>
                      <span className="text-foreground font-medium">{product.height}cm</span>
                    </div>}
                  {product.metadata && Object.keys(product.metadata).length > 0 && Object.entries(product.metadata).map(([key, value]) => <div key={key} className="flex justify-between py-2 border-b border-border">
                        <span className="text-muted-foreground capitalize">{key.replace(/_/g, ' ')}:</span>
                        <span className="text-foreground font-medium">{String(value)}</span>
                      </div>)}
                  {!product.weight && !product.length && !product.width && !product.height && (!product.metadata || Object.keys(product.metadata).length === 0) && <div className="text-center py-4 text-muted-foreground">
                      {t('productDetail.noAdditional', {
                        defaultValue: 'No additional product details available'
                      })}
                    </div>}
                </div>
              </AccordionContent>
            ...
            <AccordionItem value="shipping">
              <AccordionTrigger className="text-lg font-medium">{t('productDetail.shippingInfo', {
                    defaultValue: 'Shipping Information'
                  })}</AccordionTrigger>
              <AccordionContent>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('productDetail.standardShipping', {
                          defaultValue: 'Standard Shipping (5-7 days)'
                        })}:</span>
                    <span className="text-foreground font-medium">$9.99</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('productDetail.expressShipping', {
                          defaultValue: 'Express Shipping (2-3 days)'
                        })}:</span>
                    <span className="text-foreground font-medium">$19.99</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('productDetail.freeShipping', {
                          defaultValue: 'Free shipping'
                        })}:</span>
                    <span className="text-foreground font-medium">{t('productDetail.ordersOver', {
                          defaultValue: 'Orders over $100'
                        })}</span>
                  </div>
                  <p className="text-muted-foreground text-xs mt-4">
                    {t('productDetail.shippingNote', {
                        defaultValue: 'All orders are processed within 1-2 business days. You will receive a tracking number once your order has shipped.'
                      })}
                  </p>
                </div>
              </AccordionContent>
      ...
      {relatedProducts && relatedProducts.length > 0 && <div className="py-16 border-t border-border/40">
          ...
          <div className="container mx-auto px-0">
            ...
            <div className="px-8 lg:px-16 mb-12">
              ...
              <p className="text-muted-foreground">
                ...
                defaultValue: 'More items from {{collection}}',
                collection: product.collection?.title || 'this collection'
              })}
              </p>
            </div>
            ...
            <div className="relative group w-full">
              ...
              skipSnaps: true
                ...
                <CarouselContent className="-mx-4 sm:-mx-6">
                  {relatedProducts.slice(0, 12).map(p => <CarouselItem key={p.id} className="px-4 sm:px-6 mx-3 basis-auto w-[390px] mobile:w-[140px] first:ml-4 sm:first:ml-6 last:mr-4 sm:last:mr-6">
                      <Link to={`/products/${p.handle}`} className="group/item block">
                        ...
                        <div className="relative overflow-hidden w-[390px] h-[300px] mobile:w-[140px] mobile:h-[238px] rounded-xl">
                          <figure className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[520px] h-[520px] mobile:w-[238px] mobile:h-[238px] md:w-[720px] md:h-[720px] transition-transform duration-700 group-hover/item:scale-110">
                            <img src={p.thumbnail || "/placeholder.svg"} alt={p.title} className="w-full h-full object-cover" onError={e => {
                          const target = e.target as HTMLImageElement;
                          target.src = "/placeholder.svg";
                        }} />
                          </figure>
                        ...
                        <div className="mt-3 px-1">
                          ...
                          {p.variants?.[0]?.calculated_price && <p className="text-xs md:text-sm text-muted-foreground mt-1">
                              {formatPrice(p.variants[0].calculated_price.calculated_amount_with_tax || p.variants[0].calculated_price.calculated_amount || 0, currency)}
                            </p>}
                        </div>
                      </Link>
                    </CarouselItem>)}
            ...
            {product.collection && <div className="text-center mt-10 px-8 lg:px-16">
                <Button onClick={() => navigate(`/collections/${product.collection?.handle || product.collection?.id}`)} variant="ghost" className="text-sm hover:bg-primary/10">
                  {t('productDetail.viewAllIn', {
                defaultValue: 'View All in {{collection}}',
    ...
    {/* Zoom Dialog */}
    <Dialog open={isZoomOpen} onOpenChange={v => {
      setIsZoomOpen(v);
      if (!v) setIsZoomed(false);
    }}>
      <DialogContent className="max-w-[95vw] md:max-w-[85vw] p-0 bg-background/90 border-0">
        <div className="relative w-full h-[80vh] md:h-[85vh] flex items-center justify-center overflow-hidden">
          <img src={selectedImage || product?.thumbnail || "/placeholder.svg"} alt={product?.title || "Product image"} className={`max-w-full max-h-full object-contain transition-transform duration-300 ${isZoomed ? 'scale-125 cursor-zoom-out' : 'scale-100 cursor-zoom-in'}`} onClick={() => setIsZoomed(z => !z)} onError={e => {
            const target = e.target as HTMLImageElement;
            target.src = "/placeholder.svg";
          }} />
        </div>
