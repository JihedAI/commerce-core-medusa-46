...
export default function ProductDetail() {
  ...
  const { data: relatedProductsData } = useRelatedProducts(product?.collection?.id || "", region?.id, 6);
        ...
        <div className="p-4 md:p-8 min-h-screen">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8">
            ...
            <div className="w-full md:w-[40%] space-y-6 md:space-y-8">
              {/* Product Name */}
              <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-3 md:mb-4">{product.title}</h1>
                      ...
                      price.calculated_amount_with_tax !== price.original_amount_with_tax && (
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm font-medium text-green-600">
                            Save{" "}
                            {Math.round(
                              ((price.original_amount_with_tax - price.calculated_amount_with_tax) /
                                price.original_amount_with_tax) *
                                100,
                            )}
                            %
                    ...
                    {price.original_amount && price.calculated_amount !== price.original_amount && (
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm font-medium text-green-600">
                          Save{" "}
                          {Math.round(
                            ((price.original_amount - price.calculated_amount) / price.original_amount) * 100,
                          )}
                          %
                        </span>
                      </div>
                  ...
                  <div className="flex gap-3">
                    {product.variants.map((variant, index) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-full border-2 transition-all ${selectedVariant?.id === variant.id ? "border-foreground shadow-lg" : "border-border hover:border-muted-foreground"}`}
                        style={{
                          backgroundColor:
                            variant.options?.find((opt) => opt.option?.title?.toLowerCase() === "color")?.value ||
                            `hsl(${index * 60}, 50%, 60%)`,
                        }}
                      />
                    ))}
                  ...
                  <AccordionContent>
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">
                          {t("productDetail.standardShipping", {
                            defaultValue: "Standard Shipping (5-7 days)",
                          })}
                          :
                        </span>
                        <span className="text-foreground font-medium">7 TND</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">
                          {t("productDetail.expressShipping", {
